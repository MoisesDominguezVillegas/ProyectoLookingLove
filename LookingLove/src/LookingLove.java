...
public class LookingLove {
	...
	public Persona recogeDatosPersona() {
		...
		try {
			...
			} while (!complexionStr.equalsIgnoreCase("delgado") && !complexionStr.equalsIgnoreCase("grande") && !complexionStr.equalsIgnoreCase("fuerte")
					&& !complexionStr.equalsIgnoreCase("atleta"));
			...
			} while ( !colorStr.equalsIgnoreCase("rubio") && !colorStr.equalsIgnoreCase("moreno") && !colorStr.equalsIgnoreCase("pelirrojo") && !colorStr.equalsIgnoreCase("castaño")
			...
			} while (!orientacionStr.equalsIgnoreCase("hombre") && !orientacionStr.equalsIgnoreCase("mujer") && !orientacionStr.equalsIgnoreCase("ambos"));
	...
	private int contarAficiones(Enum<Aficiones>[] aficiones, int i) {
		...
		// convierto el array de enum a array String
		String[] aficionesString = new String[aficiones.length];
		int pos=0;
		for (Enum<Aficiones> aficion : aficiones) {
			aficionesString[pos] = aficion.toString();
		}
		
		
		for (String aficion : aficionesString) {
			if(miembro[i].getA)
		}
